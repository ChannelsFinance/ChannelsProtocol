...
contract Comptroller is ComptrollerV3Storage, ComptrollerInterface, ComptrollerErrorReporter, Exponential {
    ...
    event DistributedSupplierCan(CToken indexed cToken, address indexed supplier, uint canDelta, uint canSupplyIndex);

    /// @notice Emitted when Channels is distributed to a borrower
    event DistributedBorrowerCan(CToken indexed cToken, address indexed borrower, uint canDelta, uint canBorrowIndex);
    ...
    function updateCanSupplyIndex(address cToken) internal {
        ...
        if (deltaBlocks > 0 && supplySpeed > 0) {
            ...
            Double memory ratio = supplyTokens > 0 ? fraction(canAccrued, supplyTokens) : Double({mantissa: 0});
    ...
    function updateCanBorrowIndex(address cToken, Exp memory marketBorrowIndex) internal {
        ...
        if (deltaBlocks > 0 && borrowSpeed > 0) {
            ...
            Double memory ratio = borrowAmount > 0 ? fraction(canAccrued, borrowAmount) : Double({mantissa: 0});
     ...
     */
    function claimCan(address[] memory holders, CToken[] memory cTokens, bool borrowers, bool suppliers) public {
